  ...
  <div class="min-h-screen bg-slate-50 pb-24">
    ...
    <div class="max-w-6xl mx-auto px-6 py-6">
      ...
      <div class="flex gap-2 overflow-x-auto pb-2 mb-6 no-scrollbar">
        <button
          v-for="s in [null, 5, 4, 3, 2, 1]"
          :key="s"
          @click="starFilter = s; fetchHotels()"
      ...
      <!-- Grid -->
      <div v-else-if="hotels.length" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        <router-link
          v-for="hotel in hotels"
          :key="hotel.id"
          :to="{ name: 'hotel.detail', params: { slug: hotel.slug } }"
        ...
        >
          <div class="h-48 bg-slate-100 overflow-hidden">
            ...
            />
            <Building2 v-else class="w-16 h-16 text-slate-300 mx-auto mt-16" />
          </div>
          <div class="p-5">
            <div class="flex items-center gap-1 mb-1">
              <Star v-for="n in (hotel.star_rating || 0)" :key="n" class="w-3 h-3 text-amber-400 fill-amber-400" />
            </div>
            <h3 class="text-sm font-black text-slate-800 mb-1 group-hover:text-blue-950 transition line-clamp-1">{{ hotel.name }}</h3>
            <p class="text-xs text-slate-400 flex items-center gap-1 mb-3">
              <MapPin class="w-3 h-3" /> {{ hotel.address }}
            </p>
            <p class="text-base font-black text-blue-950">
              Mulai <span class="text-xs font-bold text-slate-400">dari</span> Rp {{ Number(hotel.min_price).toLocaleString('id-ID') }}
              <span class="text-[10px] font-medium text-slate-400">/malam</span>
            </p>
          </div>
        </router-link>
      </div>
      ...
      <div v-else class="text-center py-20">
        ...
        <p class="text-sm text-slate-400 font-medium">Hotel tidak ditemukan</p>
      </div>

      <!-- Pagination -->
      <div v-if="meta.lastPage > 1" class="flex justify-center gap-2 mt-10">
        <button
          v-for="p in meta.lastPage" :key="p"
          @click="page = p; fetchHotels()"
          class="w-10 h-10 rounded-xl text-sm font-bold border transition"
          :class="page === p
            ? 'bg-blue-950 text-white border-blue-950'
            : 'bg-white text-slate-600 border-slate-200 hover:border-blue-950'"
        >{{ p }}</button>
...
import { Building2, Star, MapPin, Search, ArrowLeft } from 'lucide-vue-next'
...
const debouncedFetch = () => {
  ...
  debounceTimer = setTimeout(() => { page.value = 1; fetchHotels() }, 400)
}
...
const fetchHotels = async () => {
  ...
  try {
    ...
    if (starFilter.value) params.star_rating = starFilter.value
